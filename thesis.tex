...
\begin{itemize}
	...
	\item El \textit{chatbot} permite crear un perfil de usuario, el cual contiene información inicial del mismo. Esta información ayuda al \textit{chatbot} a realizar recomendaciones basadas en preferencias como el género de las películas y datos del propio usuario. Esto permite abordanr el problema del arranque en frío en sistemas de recomendación colaborativa, donde el sistema aun no tiene información sobre el comportamiento del usuario, ya que este no aun no ha interactado con el mismo.
...
general están diseñados para probar modelos de recomendación. Por otro lado, es
...
El conjunto de datos \textit{MovieLens 25M}~\cite{movielens} prácticamente no tiene características
...
El conjunto de datos \textit{TMDB}~\cite{tmdb} no tiene calificaciones personalizadas para los ítems como sucede con el conjunto de datos anterior (\textit{MovieLens}) , pero si tiene varias características referentes a películas, Este conjunto de datos es utilizado para filtra los items del conjunto de datos \textit{MovieLens} ~\cite{movielens} y así tener una base de ítems mas rica en características. Estas características son utilizados en el proceso de búsqueda de ítems por el \textit{chatbot} utilizando la técnica \textit{RAG}(\textit{Retrieval Augmented Generation} o Generación Aumentada por Recuperación) ~\cite{rag} que se explicara mas adelante. El conjunto contiene datos de $5.000$ películas y sus columnas se especifican en la tabla~\ref{table:tmdbColumns} a
continuación:
